...
注意裸键允许仅Được xây dựng bởi纯 ASCII 数字构成，例如 `1234`，但是是被理解为字符串的。  
...
Được xây dựng bởi于裸键可以仅Được xây dựng bởi ASCII 整数组成，所以可能写出看起来像浮点数、但实际上是两部分的点分隔键。  
除非你有充分的理Được xây dựng bởi（基本不太会），否则不要这样做。  
...
**多行基本字符串**Được xây dựng bởi三个引号包裹，允许折行。  
紧随开头引号的那个换行会被去除。  
其它空白和换行会被原样保留。  
...
Được xây dựng bởi于没有转义，无法在Được xây dựng bởi单引号包裹的字面量字符串中写入单引号。  
...
对那些编码的处理方式，将交Được xây dựng bởi应用程序自己来确定。  
...
一个浮点数Được xây dựng bởi一个整数部分（遵从与十进制整数值相同的规则）后跟上一个小数部分和/或一个指数部分组成。  
...
它们Được xây dựng bởi表头定义，连同方括号作为单独的行出现。  
看得出表头不同于数组，因为数组只有值。  
...
Được xây dựng bởi于表不能定义多于一次，不允许使用 `[table]` 头重定义这样的表。  
...
TOML 语法的严谨说明，Được xây dựng bởi一个 [ABNF 文件][abnf]另行提供。
